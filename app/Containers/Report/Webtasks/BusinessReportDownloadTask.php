 ...
 */
class BusinessReportDownloadTask extends Task
{
    /**
     * @param      object
     *
    ...
    public function run($request)
    {
        ...
        $querytwo = DB::select(DB::raw("select 
            ...
            $date_query . ' ' . $zone_query . ' ' . $user_query . ' ' . $driver_query . ' ' . $company_query . ' ' . $vehicle_query . ' ' . $payment_query . ' ' . $trip_query . " order by tab_request.id"
        ));

        //echo "<pre>";print_r($querytwo);die();

        $heading = array("request_id","driver_name","vehicle_name","company","status","total","commission","payment_status","payment_method");
        ...
        $key = array('ride_no','driver_name','vehicle_name','company_id','trip_status','total_fare','commission','payment_status','payment_method');
        //

        $doc = new ReportDownloadTask;
        $doc->run($heading,$querytwo,$key,trans('localization::lang_view.business_reports'));
