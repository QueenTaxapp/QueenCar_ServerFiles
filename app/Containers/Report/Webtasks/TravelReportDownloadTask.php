 ...
 */
class TravelReportDownloadTask extends Task
{
    /**
     * @param      object
     *
    ...
    public function run($request)
    {
        ...
        if ($request->has('vehicle')) {
            ...
            $vehicle_query = " AND tab_Types.id = " . $vehicle . "";
        } else {
            $vehicle_query = " ";
        }
        ...
        if ($request->has('rating')) {
            ...
            $rating_query = " AND tab_request.user_rated = " . $rating . " ";
        } else {
            $rating_query = " ";
        }

        // print_r($rating_query);die();
        ...
        $querytwo = DB::select(DB::raw("select 
                                           ...
                                           tab_Types.id as vehicle_id,tab_Types.name as vehicle_name
                                           FROM tab_request
                                                 ...
                                                 left JOIN tab_Types on tab_Drivers.type =  tab_Types.id
                                                  where tab_request.deleted_at='' AND " .' ' .
            $date_query . ' ' . $zone_query . ' ' . $user_query . ' ' . $driver_query . ' ' . $vehicle_query . ' ' . $rating_query . ' ' . $payment_query . ' ' . $trip_query . " order by tab_request.id"
        ));

        //echo "<pre>";print_r($querytwo);die();

        $heading = array("request_id","user_name","driver_name","vehicle_name","status","total","commission","promo_bonus","referral_bonus","wallet_debit","driver_amount","payment_status","payment_method","driver_rating");
        ...
        $key = array('ride_no','user_name','driver_name','vehicle_name','trip_status','total_fare','commission','promo_amount','referral_amount','wallet_amount','driver_amount','payment_status','payment_method','driver_review');
        //

        $doc = new ReportDownloadTask;
        $doc->run($heading,$querytwo,$key,trans('localization::lang_view.travel_reports'));
