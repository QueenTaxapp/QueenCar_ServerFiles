...
class ApiatoExceptionsHandler extends LaravelExceptionHandler
{
    ...
    private function renderJson($exception, $request)
    {
        ...
        if(method_exists($exception,'getCustomStatusCode'))
        {
            $responseMessage['error_code'] = $exception->getCustomStatusCode();
        }
        else{
            $responseMessage['error_code'] = 500;
        }
        ...
        $responseMessage['error_message'] = $exception->getMessage();
        ...
        if ($exception instanceof \Twilio\Exceptions\RestException) {
            ...
            $responseMessage['error_message'] = trans('localization::errors.701');
            $responseMessage['error_code'] = 701;
        }

        if ($exception instanceof \Illuminate\Validation\ValidationException) {
            //$responseMessage['status_code'] = 422;
            $responseMessage['error_message'] = $exception->validator->errors()->getMessages()[key($exception->validator->errors()->getMessages())][0];
        }
        ...
        //set default status code for response
